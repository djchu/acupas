...
void print_usage(void)
{
    printf("ACUPA: Accelerated CUtting Plane Algorithm for Support Vector Machines\n"
           ...
           "         " OCAS_VERSION "\n"
                                    ...
                                    "         test_file     file with testing examples stored in SVM^light format\n"
                                    "General options:\n"
                                    "         -h            this help\n"
                                    ...
                                    "         -m [0,1,2]      solver to be used: 0 ... standard cutting plane (equivalent to BMRM, SVM^perf)\n"
                                    "                                          1 ... OCAS (default 1).\n"
                                    "                                          2 ... ACUPA.\n"
...
int main(int argc, char *argv[])
{
  ...
  ------------------------------------------------------------*/
    ...
    test_fname = calloc(len+1, sizeof(char));
    strcpy(test_fname, argv[argc-1]);
  ...
  -------------------------------------------------------------------*/
     ...
     * Load testing examples
      -------------------------------*/
    load_time = get_time();
    if(load_svmlight_file_test(test_fname,verb)==-1
            || test_X==NULL || test_y==NULL)
        goto clean_up;
    load_time = get_time()-load_time;

    nDim_test = mxGetM(test_X);
    if(nData_test == -1)
        nData_test = mxGetN(test_X);
    else if(nData_test <= 0 || nData_test > mxGetN(test_X))
    {
        fprintf(stderr,"Number of examples in the testing file is %d.\n", mxGetN(test_X));
        goto clean_up;
    }
  ...
  -------------------------------------------------------------------*/
    if(verb)
    {
        printf("Input file statistics:\n"
               "   # of training/testing examples  : %d/%d\n"
               "   trn/tst dimensionality : %d/%d\n",
               mxGetN(data_X), mxGetN(test_X), nDim, nDim_test);
